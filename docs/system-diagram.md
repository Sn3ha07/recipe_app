...
flowchart TD
    ...
    J["9. If the AI can't answer<br/>Recipes: a short message and a Try again button.<br/>Receipts and swaps: fixed sample items"]
...
- **Receipt scanner and swap search:** their helper only runs on a developer's own computer. On the live site the receipt scanner shows an error message, and the swap search does nothing (the six built-in swaps still show).
