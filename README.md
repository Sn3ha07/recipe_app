...
The AI is reached through a small helper that holds a private key. The key is kept out of this GitHub project on purpose. For recipes, the helper waits up to about 25 seconds for Gemini. If the key is missing, or Gemini fails or is too slow, the Recipes screen shows a short message and a **Try again** button. It does not fall back to made-up or pre-written recipes.
